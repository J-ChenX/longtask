...
一个 [Claude Code](https://claude.ai/code) 技能，用于编排跨多个上下文窗口的长任务，或在多个智能体会话间需要保持一致架构决策的场景。
...
longtask 提供 6 个场景化入口。根据当前状态选择正确的入口：
...
| 第三阶段：文档化 | 发现记录 + ARCHITECTURE.md | 模块文档（每个模块 overview.md + architecture.md）+ 工作包 | 用户批准文档结构 |
...
docs/
  ...
  modules/
    {模块名}/
      overview.md                 ← L1：业务视角（职责、流程、规则）
      architecture.md             ← L1：架构视角（接口、组件、数据流）
      internals.md                ← L2：实现细节（文件结构、配置、已知坑）
      security.md                 ← L2：安全细节（权限、数据分类）
      algorithm.md                ← L2：算法细节（按需，仅非平凡算法）
  appendix/
    global-concerns.md            ← 跨模块关注点（基础设施、部署、监控）
```

**L0 是项目入口。** 5 分钟读完 `ARCHITECTURE.md` 即可理解项目全貌。
**L1 是模块血肉。** 每个模块一个目录——读 `overview.md` 了解它做什么。
**L2 是模块神经。** 需要实现细节时才深入 `internals.md`。

文档按**模块**组织（而非按领域）。每个模块的业务、架构、安全、实现细节分别放在模块目录下的不同文件中，通过 markdown 交叉引用连接。
